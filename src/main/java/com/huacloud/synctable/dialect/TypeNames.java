...
public class TypeNames {
	...
	private static String replace(String type, long size, int precision, int scale) {
		...
		type = StringUtils.replaceOnce(type, "$p", Integer.toString(precision));

		if (precision == 0 && scale == 0) {
			type = StringUtils.replaceOnce(type, "(0,0)", "");
		}

		if (StringUtils.containsIgnoreCase(type, "(0)")) {
			type = StringUtils.remove(type, "(0)");
		}
